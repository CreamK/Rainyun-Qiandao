...
# --- START OF FIX: 增强 process_captcha 逻辑 (添加重试刷新) ---
def process_captcha():
    # 最大重试次数，防止无限循环
...
def get_current_points(driver: WebDriver, wait: WebDriverWait) -> int:
    ...
    try:
        ...
        return 0
# --- END OF FIX: 新增获取积分 helper 函数 ---


# --- START OF FIX: 改进 sign_in_account 逻辑 (精确签到按钮 & 积分对比) ---
    ...
    try:
        ...
        if "dashboard" in driver.current_url:
            ...
            for _ in range(max_click_retries):
                try:
                    ...
                    # 寻找包含 '每日签到' 文本的父级 div 下的 '领奖励' 按钮
                    # 这个 XPath 应该能找到按钮，无论卡片是否展开 (截图显示是展开的结构)
        ...
        if driver:
            ...
            try:
                ...
                pass
# --- END OF FIX: 改进 sign_in_account 逻辑 ---
